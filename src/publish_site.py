...
async function showSymbolDetails(symbol) {{
    ...
    modalTitle.textContent = `${{symbol}} - Last 10 Trading Days`;
    ...
    if (!apiAvailable) {{
        modalContent.innerHTML = `
            <div class="warning">
                ...
                <p>For detailed historical data and 10-day OHLC analysis, please start the API server:</p>
    ...
    modalContent.innerHTML = '<p>Loading 10-day OHLC data...</p>';
    
    try {{
        const response = await fetch(`${{apiBaseUrl}}/symbol/${{symbol}}/last10days`);
        if (!response.ok) {{
            throw new Error('Failed to load 10-day data');
        }}
        
        const result = await response.json();
        render10DayOHLCTable(result.data, symbol);
    }} catch (error) {{
        modalContent.innerHTML = `<p class="error">Error loading 10-day data: ${{error.message}}</p>`;
    }}
}}

function render10DayOHLCTable(data, symbol) {{
    const modalContent = document.getElementById('modalContent');
    
    if (!data || data.length === 0) {{
        modalContent.innerHTML = `<p class="error">No data available for ${{symbol}}</p>`;
        return;
    }}
    ...
    const totalChange = latestData.close - oldestData.close;
    const totalChangePct = ((totalChange / oldestData.close) * 100).toFixed(2);
    ...
    let html = `
        <div style="margin-bottom: 20px; padding: 12px; background: #f0f9ff; border-radius: 6px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 13px;">
                <div><strong>Period Change:</strong> ₹${{totalChange.toFixed(2)}} (${{totalChangePct}}%)</div>
                <div><strong>Average Volume:</strong> ${{Math.round(avgVolume).toLocaleString()}}</div>
                <div><strong>Highest:</strong> ₹${{maxHigh.toFixed(2)}}</div>
    ...
    // Add trend analysis
    const trend = totalChange > 0 ? 'Upward' : totalChange < 0 ? 'Downward' : 'Sideways';
    const trendColor = totalChange > 0 ? '#10b981' : totalChange < 0 ? '#ef4444' : '#6b7280';
    
    html += `
        <div style="margin-top: 16px; padding: 10px; background: #f9fafb; border-radius: 4px; text-align: center;">
            <strong>10-Day Trend: </strong>
            <span style="color: ${{trendColor}}; font-weight: 600;">${{trend}}</span>
            (${{totalChangePct}}% over 10 trading days)
        </div>
    `;
